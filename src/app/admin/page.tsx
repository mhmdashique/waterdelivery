...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFEFE] flex overflow-hidden font-dm-sans selection:bg-blue-100 selection:text-blue-600">
      ...
      {/* Sidebar */}
      <aside 
        className={`fixed left-4 top-4 bottom-4 glass-dark rounded-[2.5rem] transition-all duration-500 z-50 border border-white/20 shadow-2xl flex flex-col overflow-hidden ${sidebarOpen ? "w-72" : "w-24"}`}
        style={{ background: "rgba(255, 255, 255, 0.8)", backdropFilter: "blur(20px)" }}
      >
        <div className="p-8 flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center shrink-0 shadow-lg shadow-blue-200">
          ...
          <AnimatePresence>
            {sidebarOpen && (
              <motion.div 
                ...
                exit={{ opacity: 0, x: -10 }}
                className="font-bold text-2xl tracking-tight text-slate-900 font-syne whitespace-nowrap"
              >
                Aqua<span className="text-blue-600">Admin</span>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4">
          {[
            ...
            { id: "Pending", label: "Live Orders", icon: <Clock size={20} /> },
            { id: "Delivered", label: "Order History", icon: <CheckCircle size={20} /> },
            { id: "Customers", label: "Customer Base", icon: <Users size={20} /> },
          ].map((item) => (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id as any)}
              className={`w-full flex items-center gap-4 px-5 py-4 rounded-3xl transition-all duration-300 font-bold text-sm relative group ${
                activeTab === item.id 
                  ? "bg-slate-900 text-white shadow-xl shadow-slate-200" 
                  : "text-slate-400 hover:text-slate-900 hover:bg-slate-50"
              }`}
            >
              <span className={`shrink-0 transition-transform duration-300 ${activeTab === item.id ? "" : "group-hover:scale-110"}`}>{item.icon}</span>
              {sidebarOpen && <span>{item.label}</span>}
              {activeTab === item.id && !sidebarOpen && (
                <div className="absolute right-[-8px] w-2 h-8 bg-blue-600 rounded-full" />
              )}
            </button>
          ))}
        </nav>
        ...
        <div className="px-4 pb-8 space-y-4">
          <button
            ...
            className="w-full flex items-center justify-center p-4 rounded-3xl bg-slate-50 text-slate-400 hover:text-slate-900 transition-all border border-transparent hover:border-slate-200"
          >
            <ChevronRight size={20} className={`transition-transform duration-500 ${sidebarOpen ? "rotate-180" : ""}`} />
          </button>
          ...
          <button
            ...
            className={`w-full flex items-center gap-4 px-5 py-4 rounded-3xl transition-all duration-300 font-bold text-sm text-rose-500 hover:bg-rose-50 border border-transparent hover:border-rose-100 ${!sidebarOpen && "justify-center"}`}
          >
      ...
      {/* Main Content */}
      <main className={`flex-1 transition-all duration-500 ${sidebarOpen ? "ml-[310px]" : "ml-[120px]"} mr-4 py-4`}>
        {/* Header */}
        <header className="h-24 glass rounded-[2.5rem] border border-white shadow-sm px-8 flex items-center justify-between mb-8 sticky top-4 z-40 bg-white/60 backdrop-blur-xl">
          <div className="flex items-center gap-6 flex-1 max-w-2xl">
            <div className="relative w-full group">
              <input 
                ...
                className="w-full bg-slate-50/50 border border-slate-100 focus:bg-white focus:ring-4 focus:ring-blue-500/5 rounded-3xl py-4 pl-14 pr-6 text-sm font-semibold transition-all outline-none"
              />
          ...
          <div className="flex items-center gap-4">
            <motion.button 
              ...
              onClick={() => refreshData()} 
              className="w-12 h-12 flex items-center justify-center rounded-2xl text-slate-400 hover:bg-white hover:text-blue-600 transition-all border border-transparent hover:border-slate-100 shadow-sm"
            >
              <RefreshCw size={22} />
            </motion.button>
            <button 
              onClick={() => setIsModalOpen(true)} 
              className="flex items-center gap-3 bg-gradient-to-br from-blue-600 to-indigo-700 text-white px-8 py-4 rounded-[1.5rem] text-sm font-bold shadow-xl shadow-blue-200 hover:scale-[1.02] active:scale-[0.98] transition-all"
            >
        ...
        <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden mb-8">
          <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-white">
            ...
            <div className="flex items-center gap-3">
               ...
               {activeTab === "Customers" && (
                 <button className="flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-[1rem] text-xs font-bold hover:bg-black transition-all shadow-lg shadow-slate-200">
                      ...
                      <tr key={order.id} className="hover:bg-blue-50/20 transition-colors group">
                        ...
                        <td className="px-10 py-8">
                          ...
                          <select
                            ...
                            className={`text-[10px] font-black uppercase tracking-widest rounded-xl px-4 py-1.5 outline-none cursor-pointer border transition-all appearance-none text-center ${
                        ...
                        <td className="px-10 py-8">
                          <select
                            ...
                            className={`text-[10px] font-black uppercase tracking-widest px-4 py-2.5 rounded-[1rem] border outline-none cursor-pointer transition-all appearance-none min-w-[120px] text-center ${
                              order.status === 'Delivered' ? 'bg-blue-600 text-white border-blue-600 shadow-lg shadow-blue-100' : 
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="relative bg-white rounded-[3.5rem] shadow-2xl w-full max-w-2xl overflow-hidden border border-white">
              ...
              <div className="p-12 space-y-10 max-h-[70vh] overflow-y-auto">
                ...
                <div className="p-8 bg-amber-50 rounded-[2rem] border border-amber-100/50">
                  ...
                  <div className="mt-4 pt-4 border-t border-amber-100 flex items-center gap-3">
